 ...
 * <p>
 * Metadata is free to depend on other metadata, including both metadata classes
 * and metadata instances being depended on or depending on other metadata
 * classes and metadata instances. In other words, metadata dependency graphs
 * are not limited to only metadata instances.
 * <p>
 * For performance and memory efficiency reasons, only metadata identification
 * strings are stored by the {@link MetadataDependencyRegistry}. In particular,
 * {@link MetadataItem} instances are definitely not stored internally - which
 * is especially applicable given they are immutable.
 * <p>
 * From a logical perspective, an item of metadata can "depend on" some other
 * piece of metadata. For example, an item of metadata representing the members
 ...
 * "depended on" by other pieces of metadata. If the relationship just
 * exemplified was declared, the metadata representing the physical source file
 * on disk would be "depended on" by the Java member metadata. We use the terms
 * "upstream" and "downstream" to refer to these dependencies. In our example we
 * would say that Java member metadata is downstream of the physical source file
 * on disk metadata. We would also say that the physical source file on disk is
 * upstream of the Java member metadata.
 * <p>
 * In terms of notifications, the {@link MetadataDependencyRegistry} maintains a
 * reference to the {@link MetadataService}, and delivers notifications to the
 * {@link MetadataService} via its extension of the
 * {@link MetadataNotificationListener} interface. Note that a
 * {@link MetadataService} may subsequently deliver notifications to
 * {@link MetadataProvider}s using the semantics defined in the contract for
...
public interface MetadataDependencyRegistry {
     ...
     * them.
     * <p>
     * Attempting to register a {@link MetadataService} using this method will
     ...
     * specified downstream dependency. This is useful if rebuilding the
     * downstream dependency metadata, and just want to clear all old dependency
     * information from the registry.
     * <p>
     * The dependency must return true if presented to
     ...
     * advance (in which case the method will not return any downstream
     * dependencies, as none are known).
     * 
     * @param upstreamDependency to find the immediate downstream items for
     *            (required)
     * @return an immutable set of dependencies (never null, but the set may be
     *         empty)
     */
    Set<String> getDownstream(String upstreamDependency);

    /**
     * Obtains a list of the immediate upstream dependencies of the indicated
     * metadata item.
     * <p>
     * The downstream dependency must return true if presented to
     * {@link MetadataIdentificationUtils#isValid(String)}.
     * 
     * @param downstreamDependency to find the immediate upstream items for
     *            (required)
     * @return an immutable set of dependencies (never null, but the set may be
     *         empty)
     */
